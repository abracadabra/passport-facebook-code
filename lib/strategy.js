  ...
  if(token){
	  oauth.get(exchange_url, token, function(err, result, res){
	  	...
	  	var token = result.split('&')[0].split('=')[1],
	  		code_url = 'https://graph.facebook.com/oauth/client_code?client_id=' + oauth._clientId + '&client_secret=' + oauth._clientSecret + '&redirect_uri=' + self.options.callbackPath;
