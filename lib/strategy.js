...
function Strategy(options, verify){
	...
	passface.call(this, options, verify);
}
  ...
  if(token)
	  oauth.get(exchange_url, token, function(err, result, res){
	  	...
	  	var token = result.split('&')[0].split('=')[1],
	  		code_url = 'https://graph.facebook.com/oauth/client_code?client_id=' + oauth._clientId + '&client_secret=' + oauth._clientSecret + '&redirect_uri=' + 'http://localhost:3000/auth/facebook/callback';
